...
# ReXGradient test inference with the best models, single GPU.
#
...
#
# Usage:
#   bash scripts/infer_rexgradient_single.sh            # full + lora
#   bash scripts/infer_rexgradient_single.sh full       # full only
#   bash scripts/infer_rexgradient_single.sh lora       # lora only
#
# Overridable env: MODEL, FULL_CKPT, LORA_CKPT, TEST_DATA,
#                  RESULT_FULL, RESULT_LORA, LOG_DIR, CUDA_VISIBLE_DEVICES
...
run_full() {
  local log_file="${LOG_DIR}/infer_rexgradient_full_${TIMESTAMP}.log"
  echo "== full (${FULL_CKPT}) ==  log: ${log_file}"
  swift infer \
    --model "${FULL_CKPT}" \
...
run_lora() {
  local log_file="${LOG_DIR}/infer_rexgradient_lora_${TIMESTAMP}.log"
  echo "== lora (${LORA_CKPT}) ==  log: ${log_file}"
  swift infer \
    --model "${MODEL}" \
    --adapters "${LORA_CKPT}" \
    --val_dataset "${TEST_DATA}" \
