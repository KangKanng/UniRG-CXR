...
# Eight-GPU LoRA SFT on ReXGradient-160K.
# Usage: WANDB_API_KEY=... bash scripts/sft_lora_8gpu_rexgradient.sh
#
...
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT}/output/rexgradient_qwen3vl8b_lora_sft3}"
LOG_DIR="${LOG_DIR:-${ROOT}/logs}"
WANDB_PROJECT="${WANDB_PROJECT:-unirg-cxr-rexgradient-sft}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen3vl8b-rexgradient-lora-sft3-bs256-lr5e-5}"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG_FILE="${LOG_DIR}/sft_lora_rexgradient_${TIMESTAMP}.log"
